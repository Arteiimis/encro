...
auto registerCapturingLogger(char const* name) -> std::pair<std::shared_ptr<spdlog::logger>, std::ostringstream*> {
...
TEST_CASE("log_tags.h: tag constants use dot-notation format", "[logging][infra]") {
    ...
    for (auto const* tag : tags) {
        ...
        CHECK(std::none_of(sv.begin(), sv.end(), [](char c) { return c >= 'A' && c <= 'Z'; }));
    }
...
TEST_CASE("LOG_INFO: source location injected in message body", "[logging][infra]") {
    ...
    logger->log(
        spdlog::source_loc{__FILE__, line, ""},
        spdlog::level::info,
        formattedMsg
    );
    logger->flush();
...
TEST_CASE("logging::setup: registers all 24 named loggers", "[logging][infra]") {
    ...
    auto const allTags = std::array<char const*, 24>{
        ...
        logtags::CORE_SCAN,       logtags::CORE_JOB,      logtags::CORE_TASK,
