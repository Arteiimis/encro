...
TEST_CASE(
  "decideCq compares ssim and xpsnr points against the mapped floor",
  "[encode-probe]"
) {
  SECTION("ssim points use the ssim floor mapping") {
  ...
  SECTION("xpsnr points use the xpsnr floor mapping") {
...
// shared via testutils::copyFakeTool), copied per role so argv[0] selects ffprobe
// vs ffmpeg. The ffmpeg side writes a fake libvmaf JSON log for scoring invocations
// when ENCRO_FAKE_FFMPEG_WRITE_VMAF=1, with scores from ENCRO_FAKE_FFMPEG_VMAF_SCORES;
// duration via ENCRO_FAKE_FFPROBE_DURATION_SECS.
...
TEST_CASE(
  ...
  // The post-encode comparison hint formats both paths for `encro preview`.
  CHECK(
