...
TEST_CASE(
  ...
  auto const toolchain = e2e::installFakeToolchain(temp.path / "fake-tools");
  auto const gateFile = temp.path / "release-gate";
  auto const invocationLog = temp.path / "invocations.log";
  auto const env = std::map<std::string, std::string>{
    {{
  ...
  // Both encodes are in flight before either can finish: each ffmpeg
  // invocation logs its argv and then blocks on the gate file, so two ffmpeg
  // entries in the log prove the run started the second encode while the first
  // was still held. A wall-clock overlap window would couple the proof to
  // machine load (parallel shards, busy CI); the gate does not.
  auto const countHeldEncodes = [invocationLog] {
    auto ifs = std::ifstream{invocationLog};
    if (!ifs.is_open()) { return std::size_t{0}; }
    auto count = std::size_t{0};
    for (auto line = std::string{}; std::getline(ifs, line);) {
      if (line.starts_with("ffmpeg")) { ++count; }
    }
    return count;
  };
  auto bothInFlight = false;
  auto watcher = std::jthread{[&] {
    bothInFlight = testutils::waitUntil(
      [&] { return countHeldEncodes() >= 2; },
      std::chrono::seconds{20}
    );
    // Release the gate either way: a serialized run has to fail the check
    // below instead of hanging until the fake tool's fail-safe deadline.
    auto release = std::ofstream{gateFile};
    release << "released\n";
  }};
