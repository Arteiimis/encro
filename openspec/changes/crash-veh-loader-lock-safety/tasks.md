...
- [ ] 1.1 Add the DLL-load zone to `crash_runtime` (nestable `thread_local`
...
- [ ] 1.2 Gate `writeCrashReport`: skip `captureStacktrace` while zoned and
...
- [ ] 1.3 Gate `vehFatalHandler`: return `EXCEPTION_CONTINUE_SEARCH` silently
...
- [ ] 2.1 Wrap the `LoadLibraryExW` loop in `ensureGpuRuntimePaths`
      (onnx_tagger.cpp) with `ScopedDllLoadZone`; verify the unit suite stays
      green (`xmake test-report`)
- [ ] 2.2 Wrap the `LoadLibraryW`/`FreeLibrary` window in `hasNvidiaDriver`
...
- [ ] 3.1 Add the `--encro-crash-child=dll-zone` mode to `tests/test_main.cpp`
...
- [ ] 3.2 Add the parent integration test spawning that mode with a timed
...
- [ ] 3.3 Manual verification on this machine (model files present):
