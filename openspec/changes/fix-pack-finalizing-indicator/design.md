...
- **Verification limit (accepted)**: the rule "no indicator without a destination" is verified through its inputs (`renderable()` is false on a non-TTY stdout) and its positive direction (a status-text consumer alone keeps the indicator alive); the thread's absence itself is deliberately not asserted, because observing it would need thread introspection or production instrumentation that exists only for the test.
