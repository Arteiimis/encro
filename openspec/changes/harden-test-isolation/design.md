...
Add the missing guards next to the existing ones: a logger guard that re-installs the run's sink-less default logger after `logging::shutdown()` (used by the 25 call sites) and a crash-context guard that restores the provider. The colour-mode guard moves out of `tests/infra/terminal_tests.cpp` into `tests/test_utils.h` so `tests/cmd_cmd_tests.cpp` can drop its 7 statement-based resets; the existing `ScopedStopSignalReset`, `ScopedEnvVar` and `ScopedSyntheticJobClock` get used at the sites that still reset by statement (`tests/cmd_cmd_tests.cpp`, the watchdog case in `tests/infra/stop_signal_tests.cpp:103-124`, `tests/infra/crash_runtime_tests.cpp:175`).
