...
In the help option tables, a boolean flag that has a registered negation form SHALL render its name column with the collapsed `--[no-]name` form (for example `-p, --[no-]pack`). The negation form SHALL NOT be listed as a separate option entry.
