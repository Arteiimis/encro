...
One `ProgressContext` bar ("Analyzing", completion count, `img/s` postfix, ETA via `EtaEstimator`) for the per-image stage; stage transitions and the final report use the existing `terminal` badge styles. Copy stage gets a short bar. Non-TTY behavior inherits whatever the encode pipeline already does. Ctrl-C stops between images; the cache holds completed analysis.
