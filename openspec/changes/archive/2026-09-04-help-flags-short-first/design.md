...
- **Auto-fit measures the composed short+long column, which is equivalent to long-cell-only.** `computeMaxColumnLen` kept measuring `formatOptionName(opt) + default`, but `formatOptionName` now returns the constant 4-char short cell prepended to the long cell; a constant offset shifts every row equally, so the same option hugs the column and the observable auto-fit behavior matches the long-cell-only rule. Measuring the long cell separately was rejected: it would introduce a second width concept (`colWidth + constant`) next to the `computeColumnWidth` math shared with the commands section.
- **The old short-only fallback branch is dropped.** The former `else if (!snames.empty())` branch rendered a bare `-x` for an option with no long names; no such option is registered (every flag registration includes a long name), and a hypothetical one would render a dangling `-x, ` cell. Re-add a branch only if a short-only option is ever registered.
