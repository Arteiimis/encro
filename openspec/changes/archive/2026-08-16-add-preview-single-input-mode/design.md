...
6. **Score** each window with `videoquality::measureSegmentQuality(ffmpeg, original, seg_i, startUs, durationUs, info, /*encodedHasLocalPts=*/true)` — same call the probe phase makes.
...
8. **Summary after render**: the window list (with the worst window marked) and the `Preview written to:` line are printed only after the render completes — the bar and the render are the loading phase, the report is the outcome.

Concurrency: the window encode+score tasks are independent and run in parallel through `taskexec::runTasks`, bounded by `maxParallelJobs` (default 4); outcomes are collected in window order for the report. The probe phase itself parallelizes its base grid (3 independent CQs in wave 1; extension points stay serial because each depends on the previous decision). Stop-signal: checked after the parallel window batch completes (and after probing), returning the run as canceled.

Progress: a single progress bar spans the whole pipeline in the same style as the encode bars — probe 0-40% (per encode/score sub-step, `CQ N <phase>` in the postfix), windows 40-85% (`n/m`), render 85-100% (`Rendering comparison video...`), then 100% Success. The terminal cursor is hidden while the bar renders (`progress::CursorGuard`) and restored afterwards; non-TTY output (pipes, tests, CI) renders nothing. Probe-skip runs start the window phase at 0% instead of 40%.
