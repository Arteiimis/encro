...
The two-tier help contract (`-h` brief tier, `-hh` full tier, usage lines, option groups, hint line, `(=default)` displays) SHALL remain unchanged by this change.

#### Scenario: Brief and full help unchanged
- **WHEN** the user runs `encro -h` and `encro -hh`
- **THEN** the output matches the previous structure: same usage lines, same groups, same advanced-option hiding, same `Run 'encro -hh' to view all options.` hint, and same `(=value)` default displays