...
The badge's private `formatDurationPart` stays in `progress.cpp` and keeps its fixed-width `00m:24s` sub-minute form, because the badge scrolls and needs a stable width. Narration gets `displaytext::formatDuration(std::chrono::milliseconds)` beside the measurement helper `displaytext::elapsedSince(startedAt)` (`24s` / `12m:34s` / `1h:05m`). Sharing one function between the two renderings would need a padding mode that only the badge uses, so the two forms stay separate; the split is deliberate, not an oversight.
...
`resolvePlanNameWidth` (`src/video/encode_probe.cpp:999`) takes a third bound: `85 - kRuleFixedWidth - kRatioMarkerSlack` (34 + 2), giving a 49-column name column at most. The 2-column slack is the ratio column's ` ↑` growth marker, which renders past its 6-column header budget. Note `displaytext::layoutColumns` budgets 32 fixed columns while a row actually spends 34 — the min() absorbs the 2-column discrepancy, so `layoutColumns` is left alone and the cap is expressed against the real number. Warning rows keep their marker inside the same budget (their name cell is truncated to `nameWidth - 2`).
