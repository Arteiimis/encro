...
A phase summary line's outcome tokens carry the role of the outcome they report: the leading verb is `Good` when the phase had neither failures nor skips, `Bad` when it had at least one failure, and `Warn` when it only skipped work; a `(n failed)` segment is `Bad` and a `(n skipped)` or `(n not probed)` segment is `Warn`. The `<succeeded>/<total>` count ratio is one neutral `Accent` token — a ratio whose numerator and denominator carried different roles read as a rendering fault — so the outcome is stated by the verb and the class segments alone. The line's elapsed duration is `Accent` too. The plan block is unaffected — its header row, file rows and totals line keep the unstyled rendering they have today.
...
- **THEN** the count ratio renders as one accented token, the `(n failed)` segment red, and the `(n skipped)` segment yellow, each as its own span, with the surrounding prose unstyled
