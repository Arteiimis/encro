...
- [x] 4.4 Commit (implementation + its tests + this file's checkboxes in ONE conventional-commit), push, confirm ubuntu debug/release jobs green; download coverage artifact and compare bottom modules against baseline run 33002646675: no orchestration source file below double-digit line coverage (`preview_process.cpp` 5.4% → 84.4%, `picture_compress.cpp` 3.7% → 86.0%, `video_process.cpp` 10.4% → 74.2%); src overall 70.64% → 83.01% (run 33079034253, all jobs success).
