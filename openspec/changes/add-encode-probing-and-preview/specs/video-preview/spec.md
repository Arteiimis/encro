...
A `preview` subcommand that generates a side-by-side comparison video from representative segments of an original video and its encoded output, scored per segment, and opens the result for visual verification.

## ADDED Requirements

### Requirement: Preview subcommand compares two videos

`encro preview <original> <encoded>` SHALL accept exactly two positional video inputs (original and encoded) and generate a side-by-side comparison video.

#### Scenario: Successful preview generation
...
- **WHEN** either input does not exist or is not a video file
...
Each selected window SHALL be scored with the p5 quality metric (VMAF, SSIM fallback) computed between the original and the encoded file, and the scores SHALL be printed as a list in which the worst-scoring window is marked.
