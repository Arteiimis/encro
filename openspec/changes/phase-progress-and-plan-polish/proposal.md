...
- With colors enabled, each token of a summary line carries the role of the outcome it reports: verb Good on full success / Bad when the phase had a failure / Warn when it only skipped work, failed count Bad, skipped count Warn, and the `<succeeded>/<total>` ratio, paths and durations Accent. Nothing is colored when styling is disabled.
