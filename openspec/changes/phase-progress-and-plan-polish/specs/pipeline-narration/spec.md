...
Every phase that renders progress bars SHALL print exactly one summary line, after its bars are cleared (`progress-bar-lifecycle`). Phases that process a counted set of items state their counts, and a phase that can also fail or skip items appends one segment per non-empty class; phases that write a user-facing artifact name it; every phase's line ends with the phase's own elapsed time (`Phase summary lines state their own elapsed time`):
