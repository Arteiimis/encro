...
- [x] 1.1 Add an enumeration helper that runs the suite binary's machine-readable
...
- [x] 1.2 Add a spec-escaping helper (`,` becomes `\,`) plus a preflight that writes the
...
- [x] 1.4 Add the assignment check: after partitioning, every enumerated name must appear
...
- [x] 2.3 Spawn each shard with its spec file (`-f <suite>/shard-<i>.specs.txt`) instead
  of `--shard-count`/`--shard-index`, keeping the per-shard temp roots, the JUnit report,
  `--durations yes` and the stale-binary guard unchanged. Verify: `xmake test-parallel`
  exits 0; the unit aggregate equals the console total of
  `build\windows\x64\release\tests.exe -r console` (15594 for 723 cases here); for e2e
  the executed case count equals the enumerated 47 and the aggregate is the sum of the
  four shards' own summaries, while the e2e count itself is not conserved (719-729 in
  one process against 769-815 sharded, because its assertions depend on the shard's
  private temp root — mechanism and per-site evidence in `docs/backlog.md`).

## 3. Coverage reporting and diagnostics

- [x] 3.1 After the run, require each shard's executed case count to equal the count
  ...
  restore.
- [x] 3.2 Report one aggregate per suite as the sum of that suite's shard totals, together
  ...
  a reduced total.
- [x] 3.3 Add the `--selftest` option to the task menu, running all pure helpers
...
- [x] 4.1 After the change, three consecutive `xmake test-parallel` runs report a unit
  aggregate equal to the single-process unit total for that build (15594 each) and
  executed case counts equal to the enumerated counts for both suites, with a stable
  case count and no shard reporting zero cases. The pre-change evidence that the
  aggregate was unreliable is recorded in `proposal.md` (21113 / 11350 / 7375 / 17973
  against 16306). Verify: the runs' aggregate and case-count lines, plus the direct
  single-process totals.
- [x] 4.2 Balance: the printed max/mean shard cost is at most 1.3 for the unit suite, and
...
- [x] 4.3 Red-capability mutation: inject `CHECK(false)` into one test case, confirm the
...
- [x] 4.4 Unsharded suites are unaffected: `xmake test-report` and `xmake run e2e_tests`
...
- [x] 5.1 Update the `AGENTS.md` parallel-tests bullet: name-based partitioning, the
...
- [x] 5.2 Update the plugin's header comment, which still documents the `--shard-count`
...
- [x] 5.3 If implementation reveals drift from the design (a renamed cost file, a
