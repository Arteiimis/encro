...
- Add parsing-branch tests to `tests/video/video_info_tests.cpp`, driven through public entry points only: fraction/duration/dimension corners ("N/A", zero-denominator fractions, missing stream keys) reached by seeding `RuntimeContext::videoInfoCache` and calling `getVidTotalFrames`/`getVidTotalDurationUs` (the cache short-circuits before any ffprobe exec), plus webp input-collection boundaries via `readAllVids`/`readAllVidsFromFiles` with real temp files - table-driven, no fake tool needed for the parse cases. The `stoll`-throwing non-numeric nb_frames corner is out of scope (pre-existing production finding, reported separately).
...
- `tests/video/video_batch_execution_tests.cpp` (exists; adds batch-branch cases), `tests/video/video_encode_runner_tests.cpp` (new), `tests/video/video_info_tests.cpp`: ~+40-50 unit cases total (estimate; driven by the branch list above).
