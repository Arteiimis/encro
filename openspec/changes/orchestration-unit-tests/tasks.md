...
- [x] 2.3 Add test: target missed → `webpMinQualityFallback` engages after the quality ladder exhausts - with uniformly oversized output the invocation log shows 7 attempts (q=80..20), the final attempt carries q=20, and success is claimed via the fallback.
...
- [x] 3.3 Add test: assemble/concat failure - error surfaces to the encoding state with the assemble step identified.
...
- [x] 4.1 Seed `RuntimeContext::videoInfoCache` and drive public `getVidTotalDurationUs`; add table-driven SECTIONs for fraction parsing ("N/A", "0/0", empty, negative, oversized numerator).
