...
- **File-creation helpers**: one `writeSizedFile(path, sizeBytes)` in `test_utils.h` (sparse-friendly seek-past-end write; interior bytes unspecified, returning the path); the 4 sparse variants and exact/near copies collapse onto it, and the text-content helpers (`createFile`/`createTempFile`) collapse onto `writeTextFile`. Naming follows the existing `writeTextFile` / `touchFile` family.
