...
- **Post-Change Review:** after self-verification, unless trivial (typos, docs-only, one-liner or mechanical refactor), run the `code-review` skill (spec from `openspec/changes/`; if the current harness has no copy, use another harness's, e.g. `.pi/skills/code-review/`) with a third **leanness** sub-agent alongside Standards/Spec: ponytail-review criteria — `delete:`/`stdlib:`/`native:`/`yagni:`/`shrink:` tags, one line per finding, `net: -N lines` metric; correctness/security/performance out of scope (other axes own those); never flag the mandated test or tooling-enforced rules. Sub-agents don't inherit skills or the author's ponytail mode — paste the tag rules into its brief. For large multi-area changes add ≤1 sub-agent per functional area for edge cases only; report severity + file:line findings, triage, fix, re-verify per the review fix loop below.
