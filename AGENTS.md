...
- **Tests (parallel):** `xmake test-parallel` — unit + e2e suites in parallel shards (real-ffmpeg tests included; `--unit-shards=N` / `--e2e-shards=N` override shard counts). Each shard writes its own Catch2 JUnit report and console log; the verdict comes from the report (a missing or unreadable report, or a non-zero `failures`/`errors` count, fails the shard), while log text is evidence and `proc:wait` statuses stay untrusted. Failed shard logs and reports land under `build/.test-parallel/`.
